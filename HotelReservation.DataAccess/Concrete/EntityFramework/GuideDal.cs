...
{
    public class GuideDal : EntityRepositoryBase<Guide, ApplicationDbContext>, IGuideDal
    {
        public async Task<bool> SetActive(int id)
        {
            using (ApplicationDbContext context = new ())
            {
                var active = await context.Set<Guide>().Where(i => i.Id == id).FirstOrDefaultAsync();
        ...
        public async Task<bool> SetDeleted(int id)
        {
            using (ApplicationDbContext context = new ())
            {
                var deleted = await context.Set<Guide>().Where(i => i.Id == id).FirstOrDefaultAsync();
