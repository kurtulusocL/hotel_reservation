...
namespace HotelReservation.DataAccess.Concrete.EntityFramework
{
    public class SocialMediaDal : EntityRepositoryBase<SocialMedia, ApplicationDbContext>, ISocialMediaDal
    {
        public async Task<bool> SetActive(int id)
        {
            ...
            {
                var active = await context.Set<SocialMedia>().Where(i => i.Id == id).FirstOrDefaultAsync();
        ...
        public async Task<bool> SetDeleted(int id)
        {
            using (ApplicationDbContext context = new())
            {
                var deleted = await context.Set<SocialMedia>().Where(i => i.Id == id).FirstOrDefaultAsync();
